...
query_ = tf.placeholder(tf.float32, [None, transe_size]) #real FB representation, used in training
...
real_query = tf.select(tf.equal(choice, [0]*1000), query, temp_query) #used in testing

A = tf.Variable(tf.random_normal([lstm_size*2, transe_size], mean=0, stddev=dev))
test = tf.placeholder(tf.bool, [1000])
...
x = Q
W = tf.Variable(tf.random_normal([transe_size, transe_size], stddev=dev), name='W')
...
loss = - tf.reduce_sum(t_*tf.log(t+1e-5) + (1-t_)*tf.log(1-t+1e-5))\
	+ 0.1* (mag(A)+mag(W)+mag(T))
opt = tf.train.AdamOptimizer(0.005)
g = ['W:0', 'T:0']
grad = opt.compute_gradients(loss, var_list=[v for v in tf.global_variables() if v.name in g])
...
exclude = ['W:0', 'T:0']
vl = [v for v in tf.global_variables() if v.name not in exclude and v.name.find('Adam')==-1]
new_saver = tf.train.Saver(max_to_keep=100, var_list=vl)
...
def fdict(now, size, interval, _entity, _context, _label, _fbid, _embedding):
	...
	fd[disamb_in] = [ [1] * max_candidate ] * 1000
