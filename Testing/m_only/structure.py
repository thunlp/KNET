...
real_query = tf.select(tf.equal(choice, [0]*1000), query, temp_query)
...
x = entity_drop
W = tf.Variable(tf.random_normal([r.word_size, transe_size], stddev=dev), name='W')
...
g1 = ['W:0', 'T:0']
grad1 = opt1.compute_gradients(loss1, var_list=[v for v in tf.global_variables() if v.name in g1])
